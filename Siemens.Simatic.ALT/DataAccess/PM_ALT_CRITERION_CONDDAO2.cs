...
{ 
    public partial class PM_ALT_CRITERION_CONDDAO : IPM_ALT_CRITERION_CONDDAO
    {
        public void SetDeletedByAlert(Guid alertID)
        {
            try
            {
                string sql = string.Format(@" update dbo.PM_ALT_CRITERION_COND set RowDeleted = 1 where AlertID = @AlertID", alertID);
        ...
        public void ClearDeletedByAlert(Guid alertID)
        {
            try
            {
                string sql = @" delete from dbo.PM_ALT_CRITERION_COND where AlertID = @AlertID and RowDeleted = 1";
